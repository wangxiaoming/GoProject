...
func main() {
	...
	conn := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=%s", USERNAME, PASSWORD, SERVER, PORT, DATABASE, "utf8")
	//conn := "root:root@localhsot:3306/test" //jdbc:mysql://localhost:3306/dbName?useUnicode=true&useSSL=true
	fmt.Println(conn)
